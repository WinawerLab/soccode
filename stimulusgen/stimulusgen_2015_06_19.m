...
% NOTE!! When these images were actually shown, they were presented
% with odd runs (1, 3, 5...) getting the stimuli called "second", and even
% runs getting "first". This means that confusingly, the stimuli called
% "second" were actually the ones shown in the first run

% TODO Make sure to change the filename if you want to avoid overwriting!
save('stimuli-2015-06-19_rerun.mat', 'stimuli');

%% Retroactive fixing of the format of "stimuli"
imFile = fullfile('data', 'stimuli', 'stimuli-2015-06-19.mat');
load(fullfile(rootpath, imFile), 'stimuli');

tmpFirst = permute(reshape(stimuli.first, size(stimuli.first, 1), size(stimuli.first, 2), 9, []), [1 2 4 3]);
tmpSecond = permute(reshape(stimuli.second, size(stimuli.second, 1), size(stimuli.second, 2), 9, []), [1 2 4 3]);
tmp = cat(3, tmpFirst(:, :, 1:size(tmpFirst,3)-1, :), tmpSecond(:, :, 1:size(tmpSecond,3)-1, :)); % -1 to remove blanks
